...
export default async function Home({ params }: { params: Promise<{ tenant: string }> }) {
  ...
  // Wedding vertical: dedicated home layout
  if (tc.vertical === 'wedding') {
    return <WeddingHome tc={tc} osUrl={osUrl} />;
  }
        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 text-sm">
            ...
            {["전당대회 룰", "당정분리 위반", "AI 댓글부대 의혹", "재난지원금 오보"].map(k => (
              <Link key={k} href={`/${tenantId}/search?q=${encodeURIComponent(k)}`} className="bg-slate-800 text-slate-300 px-3 py-1 rounded-full border border-slate-700 hover:border-blue-500 hover:text-blue-400 cursor-pointer transition-colors">
                {k}
              </Link>
            ))}
          </div>
